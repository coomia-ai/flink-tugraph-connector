...
| Direction | Source (bounded vertex scan) | ✅ v0.2 |
...
> v0.2 reads **vertices** only, with projection and filter (`=,<>,>,>=,<,<=,IN`) push-down applied.
> Edge source and nested types are planned.
...
- **v0.2 (in progress)** — bounded vertex `ScanTableSource` ✅, `LookupTableSource` with
  `LookupCache` ✅, projection ✅ and filter ✅ pushdown; edge source and nested types planned.
...
v0.2 仅支持读取**点**，已支持 projection 与 filter 下推；边 source、复杂类型仍在规划中。
...
- **v0.2（进行中）**：有界点 `ScanTableSource` ✅、带缓存的维表 `LookupTableSource` ✅、projection ✅ 与
  filter ✅ 下推；边 source、复杂类型规划中。
