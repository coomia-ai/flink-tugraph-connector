...
> v0.2 reads **vertices** only. Edge source, filter pushdown and nested types are planned.
...
- **v0.2 (in progress)** — bounded vertex `ScanTableSource` ✅, `LookupTableSource` with
  `LookupCache` ✅, projection pushdown ✅; filter pushdown, edge source and nested types planned.
...
v0.2 仅支持读取**点**；边 source、filter 下推、复杂类型仍在规划中。
...
- **v0.2（进行中）**：有界点 `ScanTableSource` ✅、带缓存的维表 `LookupTableSource` ✅、projection 下推 ✅；
