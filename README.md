...
| Type mapping | nested ARRAY/MAP/ROW | ❌ TuGraph stores scalar properties only |
...
> projection. TuGraph stores **scalar properties only**, so nested ARRAY/MAP/ROW types are not
> supported (serialize to a STRING column upstream if you need them).
...
projection。TuGraph 仅存标量属性，故不支持嵌套 ARRAY/MAP/ROW（如需可在上游序列化为 STRING 列）。
